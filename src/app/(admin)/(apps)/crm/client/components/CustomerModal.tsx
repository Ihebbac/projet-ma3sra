...
import { Eye, User, Phone, Tag, Calendar, MessageSquare, Box, Scale, Droplet, Percent, Gauge, DollarSign, Weight } from 'lucide-react'
...
const CustomerModal: React.FC<Props> = ({ show, onHide, onAdded, onClientSaved, user, clients }) => {
    ...
    if (clientId && clients && clients.length > 0) {
      ...
      if (selectedClient) {
        setFormValues((prev) => ({
          ...
          numTelephone: String(selectedClient.numTelephone || ''),
          type: selectedClient.type || 'فلاح',
        }))
      }
    }
  }, [clientId, clients])
  ...
  const getComputed = () => {
    ...
    const net = lastEdited === 'oliveNet' ? Number(quantiteOliveNet || 0) : Math.max(0, olive - (Number(nombreCaisses || 0) * POIDS_CAISSE))
    const oliveBrute = lastEdited === 'oliveNet' ? net + (Number(nombreCaisses || 0) * POIDS_CAISSE) : olive
    ...
    const prixFinal = Math.round(net * prixKg)

    return { net, oliveBrute, nisba, nWiba, nQfza, kattou3, prixFinal, huileLitres }
  }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const body = {
        ...
        quantiteOlive: computed.oliveBrute,
        ...
        nisbaReelle: Number(formValues.nisba) || computed.nisba,
        nombreWiba: computed.nWiba,
        nombreQfza: computed.nQfza,
        huileParQfza: computed.nQfza > 0 ? (Number(formValues.quantiteHuile) || 0) / computed.nQfza : 0,
  ...
  const resetDate = async () => {
    try {
      const res = await fetch('https://timeapi.io/api/Time/current/zone?timeZone=Africa/Tunis')
      if (res.ok) {
        const data = await res.json()
        setFormValues((prev) => ({ ...prev, dateCreation: data.dateTime.substring(0, 10) }))
        return
      }
    } catch {}
    setFormValues((prev) => ({ ...prev, dateCreation: getTodayDate() }))
  }
        ...
        <Form onSubmit={handleSubmit}>
          <Modal.Header closeButton className="border-0 pb-0">
            <div>
              ...
              <div className="small text-body-secondary mt-1">Ajouter un client à l'huilerie</div>
            </div>
          </Modal.Header>

          <Modal.Body className="py-3">
            {/* Client existant */}
            <div className="customer-card p-3 mb-3" style={{ position: 'relative', zIndex: 10 }}>
              <div className="d-flex align-items-center gap-2 mb-2">
                <Eye size={16} className="text-secondary" />
                <span className="small fw-medium text-secondary">Reprendre un client existant</span>
              </div>
                  ...
                  <Row className="g-2">
                    <Col md={6}>
                      <Form.Group>
                        ...
                        <Form.Control name="nomPrenom" value={formValues.nomPrenom} onChange={handleChange} placeholder="Ahmed Trabelsi" size="sm" required />
                      </Form.Group>
                    </Col>
                    <Col md={3}>
                      <Form.Group>
                        <Form.Label className="small text-secondary"><Phone size={12} className="me-1" />Téléphone</Form.Label>
                        <Form.Control name="numTelephone" value={formValues.numTelephone} onChange={handleChange} placeholder="96 458 362" size="sm" />
                      </Form.Group>
                    </Col>
                    <Col md={3}>
                      <Form.Group>
                        ...
                        <Form.Select name="type" value={formValues.type} onChange={handleChange} size="sm">
                          <option value="فلاح">فلاح (Fellah)</option>
                          <option value="كيال">كيال (Kayyel)</option>
                        </Form.Select>
                      </Form.Group>
                    </Col>
                    <Col md={4}>
                      <Form.Group>
                        ...
                        <div className="d-flex gap-2">
                          ...
                          />
                          <Button variant="outline-secondary" size="sm" onClick={resetDate}>Auj.</Button>
                        </div>
                      </Form.Group>
                    </Col>
                    <Col md={8}>
                      <Form.Group>
                        ...
                        <Form.Control as="textarea" rows={1} name="commentaire" value={formValues.commentaire} onChange={handleChange} placeholder="Optionnel..." size="sm" />
                      </Form.Group>
                    </Col>
                  </Row>
                </div>
